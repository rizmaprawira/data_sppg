...
DEFAULT_OUTPUT_WORKBOOK = Path("bgn_sppg_operasional_geocoded_kalimantan_roads.xlsx")
...
def determine_extract_group(province: object) -> str:
    ...
    return "kalimantan" if province_norm in KALIMANTAN_PROVINCES else "other"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Geocode Kalimantan SPPG workbook using a local roads shapefile.")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    df = df.loc[df["extract_group"] == "kalimantan"].copy()
    if df.empty:
        raise ValueError("No Kalimantan rows matched the input workbook.")
