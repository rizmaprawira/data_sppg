...
DEFAULT_OUTPUT_WORKBOOK = Path("bgn_sppg_operasional_geocoded_maluku_roads.xlsx")
...
def determine_extract_group(province: object) -> str:
    ...
    return "maluku" if province_norm in MALUKU_PROVINCES else "other"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Geocode Maluku SPPG workbook using a local roads shapefile.")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    df = df.loc[df["extract_group"] == "maluku"].copy()
    if df.empty:
        raise ValueError("No Maluku rows matched the input workbook.")
