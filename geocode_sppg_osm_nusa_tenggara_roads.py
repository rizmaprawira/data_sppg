...
DEFAULT_OUTPUT_WORKBOOK = Path("bgn_sppg_operasional_geocoded_nusa_tenggara_roads.xlsx")
...
def determine_extract_group(province: object) -> str:
    ...
    return "nusa-tenggara" if province_norm in NUSA_TENGGARA_PROVINCES else "other"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Geocode Nusa Tenggara SPPG workbook using a local roads shapefile.")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    df = df.loc[df["extract_group"] == "nusa-tenggara"].copy()
    if df.empty:
        raise ValueError("No Nusa Tenggara rows matched the input workbook.")
