...
DEFAULT_OUTPUT_WORKBOOK = Path("bgn_sppg_operasional_geocoded_papua_roads.xlsx")
...
def determine_extract_group(province: object) -> str:
    ...
    return "papua" if province_norm in PAPUA_PROVINCES else "other"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Geocode Papua SPPG workbook using a local roads shapefile.")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    df = df.loc[df["extract_group"] == "papua"].copy()
    if df.empty:
        raise ValueError("No Papua rows matched the input workbook.")
