...
DEFAULT_OUTPUT_WORKBOOK = Path("bgn_sppg_operasional_geocoded_sulawesi_roads.xlsx")
...
def determine_extract_group(province: object) -> str:
    ...
    return "sulawesi" if province_norm in SULAWESI_PROVINCES else "other"
...
def process_sulawesi_rows(df: pd.DataFrame, args: argparse.Namespace, sheet_name: str) -> None:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Geocode Sulawesi SPPG workbook using a local roads shapefile.")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    df = df.loc[df["extract_group"] == "sulawesi"].copy()
    if df.empty:
        raise ValueError("No Sulawesi rows matched the input workbook.")
    df = df.loc[:, INPUT_COLUMNS + ["source_excel_row"]].copy()

    if args.output is None:
        args.output = default_output_path()

    process_sulawesi_rows(df, args, sheet_name)
